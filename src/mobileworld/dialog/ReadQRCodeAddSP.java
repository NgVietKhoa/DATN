...
public class ReadQRCodeAddSP extends javax.swing.JFrame {

    private Webcam webcam;
    private WebcamPanel webcamPanel;
    private final ViewBanHang viewBanHang;

    public ReadQRCodeAddSP(ViewBanHang viewBanHang) {
    ...
    // Define selectedQuantities and initialize it outside the loop
                ...
                if (webcam.isOpen()) {
                    ...
                    if (image != null) {
                        ...
                        if (result != null) {
                            ...
                            // Increment quantity for the scanned QR code data
                            selectedQuantities.put(qrCodeData, selectedQuantities.getOrDefault(qrCodeData, 0) + 1);
                            for (Map.Entry<String, Integer> entry : selectedQuantities.entrySet()) {
                                String qrCodeDataImel = entry.getKey();
                                int quantity = entry.getValue();
                                viewBanHang.updateGioHangWithImel(qrCodeDataImel, quantity);
                            }
                            viewBanHang.getImelAndCTSPQR(qrCodeData);
